...
const redisClient = new ioredis_1.default("rediss://default:AeQcAAIjcDE0MjMyYTMzNDEwYzc0Y2ZiOWFkMzk1M2JlZTgwM2IwMHAxMA@helpful-polliwog-58396.upstash.io:6379");
//redisClient2 will connect to default port
const redisClient2 = new ioredis_1.default("redis://127.0.0.1:6379");
...
class SocketStore {
    ...
    async subscribeTokens(instrumentKeys) {
    ...
    static async connectWebSocket(io, wsUrl) {
        return new Promise((resolve, reject) => {
    ...
    static async initProtobuf() {
        protobufRoot = await protobuf.load(__dirname + "/MarketDataFeed.proto");
        console.log("Protobuf initialized.");
    }
    static decodeProtobuf(buffer) {
        if (!protobufRoot) {
    ...
    static async initializeMarketFeed() {
        try {
            await this.initProtobuf();
            const wsUrl = await this.getMarketFeedUrl();
            this.upstoxWs = await this.connectWebSocket(_1.io, wsUrl);
...
SocketStore.isInitialized = false; // To track WebSocket initialization
SocketStore.subscribedTokens = new Set(); // Keep track of subscribed tokens
SocketStore.access_token = "";
// import { io } from ".";
